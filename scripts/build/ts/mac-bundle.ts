...
export async function macBuild() {
	...
	for (const app of BuildConfig.mac.architecture) {
		...
		const InfoPlistTemplate = fs.readFileSync(path.resolve(__dirname,"../templates/mac/Info.plist"),"utf-8")
		...
		l.info(`mac_${app} built in ${(performance.now() - appTime).toFixed(3)}ms`)
	}
	logger.success(`Built in ${(performance.now() - initTime).toFixed(3)}ms`)
}
